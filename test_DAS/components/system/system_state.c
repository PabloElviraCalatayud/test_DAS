...
void system_state_init(void) {
  ...
  // s_state ya tiene valor inicial arriba
  s_cb_count = 0;
}
...
bool system_state_set(system_state_t new_state) {
  ...
  // Si es el mismo estado, no hacemos nada (pero no es error)
  if (s_state == new_state) {
    ESP_LOGW(TAG, "State already %d, ignoring", (int)new_state);
    xSemaphoreGive(s_mutex);
    return true;
  }

  // Validación de transiciones (tu lógica)
  switch (s_state) {
    case SYS_STATE_IDLE:
  ...
  if (!ok) {
    ESP_LOGE(TAG, "Invalid transition %d -> %d", (int)s_state, (int)new_state);
    xSemaphoreGive(s_mutex);
    return false;
  }

  ESP_LOGI(TAG, "State change: %d -> %d", (int)s_state, (int)new_state);
  s_state = new_state;

  // Copiar callbacks para llamarlos fuera del mutex (tu enfoque, correcto)
  cb_count = s_cb_count;
