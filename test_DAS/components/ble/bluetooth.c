...
/**
 * Si NO necesitas acceder a la cola desde fuera de este archivo,
 * cambia esto a `static QueueHandle_t ble_tx_queue;`
 */
...
static char ble_name[16] = "ESP32_NIMBLE";
...
static int chr_tx_access_cb(uint16_t c, uint16_t a,
                            struct ble_gatt_access_ctxt *ctxt, void *arg) {
  (void)c; (void)a; (void)ctxt; (void)arg;
  return 0;
}

static int chr_rx_access_cb(uint16_t c, uint16_t a,
                            struct ble_gatt_access_ctxt *ctxt, void *arg) {
  ...
  switch (event->type) {
    ...
    case BLE_GAP_EVENT_CONNECT:
      if (event->connect.status == 0) {
        ...
        ESP_LOGI(TAG, "Device CONNECTED, handle=%d, name=%s", conn_handle, ble_name);
    ...
    case BLE_GAP_EVENT_DISCONNECT:
      ESP_LOGW(TAG, "Device DISCONNECTED (reason=%d). Restarting advertising.", event->disconnect.reason);
...
static void ble_advertise(void) {
  ...
  fields.name_len = (uint8_t)strlen(ble_name);
  ...
  (void)ble_gap_adv_start(own_addr_type, NULL, BLE_HS_FOREVER,
...
static void ble_on_sync(void) {
  ...
  // Nombre final: DAS-XXYY (últimos 2 bytes MAC)
...
static void ble_tx_task(void *arg) {
  ...
  while (1) {
    if (xQueueReceive(ble_tx_queue, &pkt, portMAX_DELAY) == pdTRUE) {
      (void)bluetooth_notify(pkt.data, pkt.len);
...
void bluetooth_init(void) {
  ...
  // Merge: tamaño 16 (como tu compañero), sin perder tu comportamiento
...
#ifndef BLE_TX_ENQUEUE_TIMEOUT_MS
// 0 = no bloqueante (tu versión). Si quieres el comportamiento del otro, pon 100.
#define BLE_TX_ENQUEUE_TIMEOUT_MS 0
#endif
...
bool bluetooth_tx_enqueue(const uint8_t *data, uint16_t len) {
  ...
  return xQueueSend(ble_tx_queue, &pkt, pdMS_TO_TICKS(BLE_TX_ENQUEUE_TIMEOUT_MS)) == pdTRUE;
}
